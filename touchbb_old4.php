...
if ($get == 'active') {
  ...
  for ($i=0;$i<count($data);$i++) {
    ...
    $output .= ((!$i)?clean('Active Topics'):'') . $delims[1];
...
if ($get == 'unanswered') {
  ...
  for ($i=0;$i<count($data);$i++) {
    ...
    $output .= clean(1) . $delims[1];
    $output .= clean($row['topic_id']) . $delims[1];
    $output .= ((!$i)?clean('Unanswered Posts'):'') . $delims[1];
    $output .= clean($row['topic_title']) . $delims[1];
...
if ($get == 'myposts') {
  ...
  if ($user->data['is_registered']) {
    ...
    for ($i=0;$i<count($data);$i++) {
      ...
      $output .= ((!$i)?clean('My Posts'):'') . $delims[1];
      $output .= clean($row['topic_title']) . $delims[1];
...
if ($get == 'sent') {
  ...
  if ($user->data['is_registered']) {
    ...
    while (list($cntr, $msg) = @each($unread)) {
      ...
      $output .= ((!$cntr++)?clean(sprintf($strings[6], count($unread), ((count($unread)==1)?'':'s'))):'') . $delims[1];
    ...
    while (list($cntr, $msg) = @each($read)) {
      ...
      // format output
      $output .= clean($msg['msg_id']) . $delims[1];
      $output .= ((!$cntr++)?clean(sprintf($strings[7], count($read), ((count($read)==1)?'':'s'))):'') . $delims[1];
      $output .= clean($msg['message_subject']) . $delims[1];
      $output .= clean(IDsToUsers($msg['to_address'] . ':' . $msg['bcc_address'])) . $delims[1];
      $output .= clean($user->format_date($msg['message_time']));
    }
...
if ($search) {
  ...
  for ($i=0;$i<count($data);$i++) {
    ...
    $output .= clean(1) . $delims[1];
    $output .= clean($topic_id) . $delims[1];
    $output .= ((!$i)?clean('Matching Topics'):'') . $delims[1];
    $output .= clean($row['topic_title']) . $delims[1];
