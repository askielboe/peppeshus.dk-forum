    ...
    while(empty($forum_rows[0]['child']) && count($forum_rows) > 1)
    {
        ...
        foreach($leaves_forum as $node_forum_id => $leaves)
        {
            foreach($leaves as $forum_id)
            {
                ...
                $description = smiley_text($forum_rows[$forum_id]['forum_desc'], true);
                $description = generate_text_for_display($description, $forum_rows[$forum_id]['forum_desc_uid'], $forum_rows[$forum_id]['forum_desc_bitfield'], $forum_rows[$forum_id]['forum_desc_options']);
                $description = preg_replace('/<br *?\/?>/i', "\n", $description);
                
                
                $xmlrpc_forum = new xmlrpcval(array(
                    'forum_id'      => new xmlrpcval($forum_id),
                    'forum_name'    => new xmlrpcval(basic_clean($forum_rows[$forum_id]['forum_name']), 'base64'),
                    'description'   => new xmlrpcval(basic_clean($description), 'base64'),
