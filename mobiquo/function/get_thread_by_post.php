...
function get_thread_by_post_func($xmlrpc_params)
{
    ...
    $config['posts_per_page'] = isset($params[1]) ? intval($params[1]) : 20;
    ...
    $sql_array['WHERE'] = "t.topic_id = $topic_id AND (f.forum_id = t.forum_id";
    ...
    // Get post position
    $sql = 'SELECT COUNT(post_id) AS post_num
        FROM ' . POSTS_TABLE . "
    ...
    $result = $db->sql_query($sql);
    $post_num = (int) $db->sql_fetchfield('post_num');
    $db->sql_freeresult($result);
    
    $position = $post_num + 1;
    $start = $post_num - ($post_num % $config['posts_per_page']);
...
//        )));
    ...
    return new xmlrpcresp(
        new xmlrpcval(array(
            ...
            'position'       => new xmlrpcval($position > $total_posts ? $total_posts : $position, 'int'),
