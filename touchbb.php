...
// used to fight spam! If you really don't care, change
// this option to true
$forceApproval = false;
...
$appURL = 'http://itunes.apple.com/us/app/touchbb-lite/id332458253?mt=8';
...
if ($newProtocol) {
  $user->timezone = $user->dst = $config['board_timezone'] = $user->data['user_timezone'] = 0;
  $user->date_format = $user->data['user_dateformat'] = $config['default_dateformat'] = 'U';
} else {
...
if ($get == 'pm') {
  ...
  if ($user->data['is_registered']) {
    if ($config['allow_privmsg']) {
      ...
      $sql = "SELECT username,message_text,bbcode_uid,message_time,message_attachment,to_address,bcc_address,author_id FROM " . PRIVMSGS_TABLE . "," . USERS_TABLE . " WHERE user_id=author_id AND msg_id=" . $db->sql_escape($id);
      $result = $db->sql_query($sql);
      ...
      if ($row) {
        $valid = false;
        $to = array_merge(preg_split('/:/', $row['to_address']), preg_split('/:/', $row['bcc_address']));
        ...
        // allow user to read their sent PMs
            ...
            for ($x=0;$x<count($attachments[$id]);$x++) {
              ...
              if ($extensions[$attachment['extension']]['display_cat'] == ATTACHMENT_CATEGORY_IMAGE) {
                if (preg_match('/\[attachment=' . $x . ':' . $row['bbcode_uid'] . '\](.*?)\[\/attachment:' . $row['bbcode_uid'] . '\]/', $message, $matches)) {
                  $message = preg_replace('/\[attachment=' . $x . ':' . $row['bbcode_uid'] . '\](.*?)\[\/attachment:' . $row['bbcode_uid'] . '\]/is', '[img:' . $row['bbcode_uid'] . ']' . generate_board_url() . '/download/file.php?id=' . $attachment['attach_id'] . "[/img:" . $row['bbcode_uid'] . "]<br>", $message);
                } else {
                  ...
                  $message .= '[img]' . generate_board_url() . '/download/file.php?id=' . $attachment['attach_id'] . '[/img]<br>';
                }
              }
            }
          }
...
if ($get == 'unanswered') {
  ...
  $sql = "SELECT t.topic_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,topic_first_poster_name,topic_time,topic_views,$replyStr,t.forum_id" . (($user->data['is_registered']&&$config['load_db_lastread'])?',tt.mark_time':'') . " FROM (" . POSTS_TABLE . " p, " . TOPICS_TABLE . " t," . FORUMS_TABLE . " f)" . (($user->data['is_registered']&&$config['load_db_lastread'])?' LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON tt.user_id = ' . $user->data['user_id'] . ' AND t.topic_id = tt.topic_id':'') . " WHERE (t.forum_id='" . implode("' OR t.forum_id='", $forumids) . "') AND t.topic_replies = 0 AND t.topic_moved_id = 0 AND p.topic_id = t.topic_id" . $sql_approved . " AND t.forum_id=f.forum_id AND forum_password='' ORDER BY t.topic_last_post_time DESC LIMIT $page, $msgsPerPage";
...
if ($get == 'myposts') {
  ...
  if ($user->data['is_registered']) {
    ...
    $sql = "SELECT t.topic_id,t.topic_moved_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,topic_first_poster_name,topic_time,topic_views,$replyStr,t.forum_id" . (($user->data['is_registered']&&$config['load_db_lastread'])?',tt.mark_time':'') . " FROM (" . POSTS_TABLE . " p, " . TOPICS_TABLE . " t," . FORUMS_TABLE . " f)" . (($user->data['is_registered']&&$config['load_db_lastread'])?' LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON tt.user_id = ' . $user->data['user_id'] . ' AND t.topic_id = tt.topic_id':'') . " WHERE (p.forum_id='" . implode("' OR p.forum_id='", $forumids) . "') AND p.topic_id = t.topic_id" . $sql_approved . " AND topic_poster=" . $db->sql_escape($user->data['user_id']) . " AND t.forum_id=f.forum_id AND forum_password='' ORDER BY t.topic_last_post_time DESC";
...
if ($get == 'reply' || $get == 'post') {
  ...
  if ($user->data['is_registered']) {
    ...
    if ($forceApproval) {
      $post_data['topic_approved'] = true;
      $post_data['post_approved'] = true;
    }
...
if ($get == 'forum') {
  ...
  if ($forum_data['left_id'] != $forum_data['right_id'] - 1) {
    ...
    for ($i=0;$i<count($forums);$i++) {
      ...
      if (!$i) {
        $output .= clean($user->lang['SUBFORUMS']);
      }
      ...
      if ($newProtocol) {
        $output .= clean($forum['LAST_POSTER']);
      } else {
        $output .= clean(sprintf($strings[0], $forum['LAST_POSTER'], $forum['LAST_POST_TIME']));
      }

      $output .= $delims[1];

      if ($newProtocol) {
        $output .= clean($forum['POSTS']);
      } else {
      ...
      if ($newProtocol) {
        ...
        $output .= clean($forum['LAST_POST_TIME']) . $delims[1];
        $output .= clean($forum['TOPICS']);
      }
    }
  } else {
      ...
      foreach ($topic_list as $topic_id) {
        ...
        if ($oldCategory != $row['topic_type']) {
          switch ($row['topic_type']) {
            case POST_NORMAL:
              $type = ($newProtocol)?1:$strings[8];
              break;

            case POST_STICKY:
              $type = ($newProtocol)?2:$strings[9];
              break;

            case POST_ANNOUNCE:
              $type = ($newProtocol)?3:$strings[10];
              break;
        ...
        $output .= (($unread_topic)?1:0) . $delims[1];
        $output .= ($replies + 1);
...
if ($get == 'index') {
  ...
  for ($i=0;$i<count($forums);$i++) {
    ...
    if ($oldCategory != $cat_name || $i == 0) {
      $output .= clean(($cat_name)?$cat_name:$user->lang['FORUM']) . $delims[1];
    } else {
      $output .= $delims[1];
    }
    ...
    if ($newProtocol) {
      $output .= clean($forum['LAST_POSTER']) . $delims[1];
      $output .= clean($forum['POSTS']);
    } else {
      if ($forum['POSTS'] > 0 && $forum['TOPICS'] > 0) {
    ...
    if ($newProtocol) {
      ...
      $output .= clean($forum['LAST_POST_TIME']) . $delims[1];
      $output .= clean($forum['TOPICS']) . $delims[1];
    }
...
if ($get == 'status') {
  ...
  if ($addThisForumToDirectory) {
    ...
    if ($lastUpdated) {
      ...
      if (($lastUpdated + (86400 * 5)) < time()) {
        ...
        if (@touch(__FILE__)) {
          $fp = @fopen('http://www.messageforums.net/touchbb.php?get=activeforum&name=' . urlencode($config['sitename']) . '&forum=' . urlencode(generate_board_url()) . '&posts=' . urlencode($config['num_posts']) . '&users=' . urlencode($config['num_users']) . '&topics=' . urlencode($config['num_topics']), "r");
  ...
  if ($promptUsersAboutTouchBB) {
    ...
    if ($cacheDir) {
      ...
      $footer .= "var cValue='';if(document.cookie.length>0){cStart=document.cookie.indexOf('$cookieName=');if(cStart!=-1){cStart+=" . (strlen($cookieName) + 1) . ";cEnd=document.cookie.indexOf(';',cStart);if(cEnd==-1)cEnd=document.cookie.length;cValue=document.cookie.substring(cStart,cEnd)}}if(cValue!='$cookieAsked'){var exdate=new Date();exdate.setDate(exdate.getDate()+3);document.cookie='$cookieName=$cookieAsked;expires='+exdate.toUTCString();if(confirm('We have detected that you are using an '+aProds[i]+'. Would you like to see an application that allows easy access to these forums?'))location.href='" . $appURL . "';}break}}";
      $footer .= "\n</script>\n";

      // scan cache directory for our alert box
      if ($handle = @opendir($cacheDir)) {
        while (false !== ($file = @readdir($handle))) {
          if (substr($file, 0, 4) == 'tpl_' && substr($file, -23) == 'overall_footer.html.php') {
            // read cached footer file
            $fp = @fopen($cacheDir . $file, "r");
            $contents = @fread($fp, @filesize($cacheDir . $file));
            @fclose($fp);
...
function clean($str, $uid = '') {
  ...
  if ($uid) {
    ...
    $bbcode = array('/\[i(|:' . $uid . ')\](.*?)\[\/i(|:' . $uid . ')\]/is' => '<i>$2</i>',
                    ...
                    '/\[youtube(|:' . $uid . ')\]\/v\/(.*?)\?(.*?)\[\/youtube(|:' . $uid . ')\]/is' => '<div align="center"><object width="300" height="243"><param name="movie" value="http://www.youtube.com/v/$2&f=gdata_videos&c=ytapi-my-clientID&d=nGF83uyVrg8eD4rfEkk22mDOl3qUImVMV6ramM"></param><param name="wmode" value="transparent"></param><embed src="http://www.youtube.com/v/$2&f=gdata_videos&c=ytapi-my-clientID&d=nGF83uyVrg8eD4rfEkk22mDOl3qUImVMV6ramM" type="application/x-shockwave-flash" wmode="transparent" width="300" height="243"></embed></object></div>',
                    '/\[box(|:' . $uid . ')\](.*?)\[\/box(|:' . $uid . ')\]/is' => '<blockquote>$2</blockquote>',
    );
...
function displayStatus() {
  global $appName, $scriptVersion, $config, $phpEx, $phpbb_root_path, $debugMode, $appURL;
