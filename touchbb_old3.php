...
// setup variables
$debug = false;
$connectorVersion = '1.8';
$appVersion = '1.0';
$appName = 'TouchBB';
$UA = str_replace('TouchBBLite', 'TouchBB', $_SERVER['HTTP_USER_AGENT']);
$delims = array(chr(13), chr(10), chr(9));

// send your forum to our public list so others can join your forum?
// defaults to off for privacy reasons
...
$msgsPerPage = request_var('mpp', 20);
$page = (request_var('page', 1) * $msgsPerPage) - $msgsPerPage;
...
if ($get == 'active') {
  ...
  $sql = "SELECT t.topic_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,username,topic_time,topic_views,$replyStr,forum_id FROM " . TOPICS_TABLE . " t LEFT JOIN " . USERS_TABLE . " ON user_id=topic_poster WHERE (forum_id='" . implode("' OR forum_id='", $forumids) . "') AND t.topic_moved_id = 0" . $sql_approved . " ORDER BY t.topic_last_post_time DESC LIMIT $page, $msgsPerPage";
...
if ($get == 'unanswered') {
  ...
  $sql = "SELECT t.topic_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,username,topic_time,topic_views,$replyStr,t.forum_id FROM " . POSTS_TABLE . " p, " . TOPICS_TABLE . " t LEFT JOIN " . USERS_TABLE . " ON user_id=topic_poster WHERE (t.forum_id='" . implode("' OR t.forum_id='", $forumids) . "') AND t.topic_replies = 0 AND t.topic_moved_id = 0 AND p.topic_id = t.topic_id" . $sql_approved . " ORDER BY t.topic_last_post_time DESC LIMIT $page, $msgsPerPage";
...
if ($get == 'myposts') {
  ...
  if ($user->data['is_registered']) {
    ...
    $sql = "SELECT t.topic_id,t.topic_moved_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,username,topic_time,topic_views,$replyStr FROM " . POSTS_TABLE . " p, " . TOPICS_TABLE . " t LEFT JOIN " . USERS_TABLE . " ON user_id=topic_poster WHERE (forum_id='" . implode("' OR forum_id='", $forumids) . "') AND p.topic_id = t.topic_id" . $sql_approved . " AND user_id=" . $db->sql_escape($user->data['user_id']) . " ORDER BY t.topic_last_post_time DESC";
...
if ($get == 'sent') {
  ...
  if ($user->data['is_registered']) {
    ...
    $pms = get_pm_from(PRIVMSGS_OUTBOX, '', $user->data['user_id']);
    ...
    while (list($key, $msg) = each($pms)) {
      ...
      $toList = array();
      $to = explode(':', $msg['to_address'] . (($msg['bcc_address'])?':' . $msg['bcc_address']:''));
      for ($i=0;$i<count($to);$i++) {
        if ($to[$i][0] == 'u') {
      ...
      $output .= ((!$cntr++)?clean(sprintf($strings[6], count($pms), ((count($pms)==1)?'':'s'))):'') . $delims[1];
      $output .= clean($msg['message_subject']) . $delims[1];
      $output .= clean(sprintf($strings[9], IDsToUsers($msg['to_address'] . ':' . $msg['bcc_address']))) . $delims[1];
      $output .= clean($user->format_date($msg['message_time']));
    }

    // get the users sent read messages
    ...
    while (list($key, $msg) = each($pms)) {
      ...
      // format output
      $output .= clean($msg['msg_id']) . $delims[1];
      $output .= ((!$cntr++)?clean(sprintf($strings[7], count($pms), ((count($pms)==1)?'':'s'))):'') . $delims[1];
      $output .= clean($msg['message_subject']) . $delims[1];
      $output .= clean(sprintf($strings[9], IDsToUsers($msg['to_address'] . ':' . $msg['bcc_address']))) . $delims[1];
      $output .= clean($user->format_date($msg['message_time']));
    }
...
*/
    ...
    submit_post($mode, $title, $username, POST_NORMAL, $poll, $data, $update_message, 0);
    ...
    if ($file_tmp != 'none' && $file_tmp) {
      ...
      $sql = 'INSERT INTO ' . ATTACHMENTS_TABLE . " (post_msg_id,topic_id,in_message,poster_id,is_orphan,physical_filename,real_filename,download_count,attach_comment,extension,mimetype,filesize,filetime,thumbnail) VALUES (" . $db->sql_escape($post_id) . "," . $db->sql_escape($topic_id) . ",0," . $db->sql_escape($poster_id) . ",0,'" . $db->sql_escape($phy_file) . "','" . $db->sql_escape($file_name) . "',0,'','" . $db->sql_escape($ext) . "','" . $db->sql_escape($mime) . "','" . $db->sql_escape($file_size) . "','" . time() . "',0)";
      $db->sql_query($sql);
...
if ($search) {
  ...
  $sql = "SELECT topic_id,topic_moved_id,topic_title,topic_last_post_time,topic_last_poster_name,username,topic_time,topic_views,$replyStr,forum_id FROM (" . TOPICS_TABLE . ") LEFT JOIN " . USERS_TABLE . " ON user_id=topic_poster WHERE (forum_id='" . implode("' OR forum_id='", $forumids) . "') AND topic_title LIKE '%" . $db->sql_escape($search) . "%' AND topic_type IN (" . POST_NORMAL . ")$sql_approved ORDER BY topic_type DESC,topic_last_post_time DESC LIMIT $page, $msgsPerPage";
  $result = $db->sql_query($sql);
  while ($row = $db->sql_fetchrow($result)) {
...
if ($get == 'topic') {
  ...
  // join to forum table on topic forum_id unless topic forum_id is zero whereupon we join on the forum_id passed as a parameter ... this is done so navigation, forum name, etc. remain consistent with where user clicked to view a global topic
  ...
  if (!sizeof($post_list)) {
    if ($debug) {
      if ($sort_days) {
        touchbb_error('NO_POSTS_TIME_FRAME');
      } else {
        touchbb_error('NO_TOPIC');
      }
    } else {
...
if ($get == 'forum') {
  ...
  if (!($forum_data['forum_type'] == FORUM_POST || (($forum_data['forum_flags'] & FORUM_FLAG_ACTIVE_TOPICS) && $forum_data['forum_type'] == FORUM_CAT))) {
    touchbb_error('Not postable forum or showing active topics');
  }
  ...
  $sql = 'SELECT t.topic_id FROM ' . TOPICS_TABLE . " t WHERE $sql_where AND t.topic_type IN (" . POST_NORMAL . ', ' . POST_STICKY . ") $sql_approved $sql_limit_time ORDER BY t.topic_type DESC,$sql_sort_order LIMIT $page, $msgsPerPage";
  $result = $db->sql_query($sql);
...
if ($get == 'status') {
  ...
  // did the user turn on this option? defaults to off
  if ($addMyForumToDirectory) {
    // send the details
    $handle = @fopen('http://www.messageforums.net/touchbb.php?get=activeforum&forum=' . urlencode(str_replace(array('http://', 'https://'), '', generate_board_url())) . '&posts=' . urlencode($config['num_posts']) . '&users=' . urlencode($config['num_users']) . '&topics=' . urlencode($config['num_topics']), "r");
    @fclose($handle);
  }
}
...
function displayStatus() {
  ...
  if ($works) {
    ...
    if ($config['cookie_domain'] != $_SERVER['HTTP_HOST']) {
      $issues .= '<li>Your cookie domain is set to <b>' . $config['cookie_domain'] . '</b>, but it should be set to <b>' . $_SERVER['HTTP_HOST'] . '</b></li>';
    }

    if ($config['script_path'] != $path) {
      $issues .= '<li>Your script path is set to <b>' . $config['script_path'] . '</b>, but it should be set to <b>' . $path . '</b></li>';
    }

    if ($config['cookie_path'] != $path) {
      $issues .= '<li>Your cookie path is set to <b>' . $config['cookie_path'] . '</b>, but it should be set to <b>' . $path . '</b></li>';
    }

    if ($issues) {
      echo "<br><br>This script has found " . ((substr_count($issues, '<li>')==1)?'a problem':'a few problems') . ' with your forum settings which may prevent TouchBB from logging in and making posts.';
